...
function CreateTransitModal({
  ...
  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 backdrop-blur-md z-40" onClick={handleCancel}></div>
      
      {/* Modal */}
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl border border-gray-200">
          {/* Header */}
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex items-center gap-2 mb-2">
              <Scan size={18} />
              <h3 className="font-medium text-gray-900">Scan Crates or Items</h3>
            </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {currentAvailableItems.length > 0 ? (
                        currentAvailableItems.map((item) => {
                          ...
                          return (
                            <tr key={code} className={`transition-colors ${isSelected ? 'bg-green-50 border-green-200' : 'hover:bg-gray-50'}`}>
